...
public class BankingSystem {
    ...
    public void createAccount(String accountNumber, String ownerName, double initialBalance) 
        ...
        if(initialBalance < 0) {
            throw new IllegalArgumentException("초기 잔액이 0 미만이 되면 안됩니다.");
        }
    ...
    public BankAccount getAccount(String accountNumber) throws InvalidAccountException {
        ...
        if (targetAccountNumber == null || targetAccountNumber.isEmpty()) {
            throw new InvalidAccountException("존재하지 않는 계좌번호 입니다.", accountNumber);
        }

        System.out.println(accounts.get(targetAccountNumber).toString());
        return accounts.get(targetAccountNumber); // 학생이 구현해야 하는 부분
    }
    ...
    public void transfer(String fromAccountNumber, String toAccountNumber, double amount) 
        ...
        if (amount < 0) {
            throw new IllegalArgumentException("이체 금액이 0원 이하입니다.");
        }

        fromAccount.withdraw(amount);
        toAccount.deposit(amount);

        System.out.println("이체에 성공 했습니다.");
    }
